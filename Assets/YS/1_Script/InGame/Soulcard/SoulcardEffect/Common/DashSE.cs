...
namespace YS
{
    public abstract partial class Unit
    {
        public partial class Skill
        {
            ...
            {
                private class SDashSE : SoulCardEffect
                {
                    private List<uint> addMaxChargeCount = new List<uint>();

                    public SDashSE(SDashSEData data, PlayableUnit skillOwner) : base(data, skillOwner)
                    {
                    ...
                    {
                        if (CurrentStack == 0)
                            caster.dashSkill.activeSkill.maxCharge += (int)addMaxChargeCount[CurrentStack];
                        else
                            caster.dashSkill.activeSkill.maxCharge += (int)(addMaxChargeCount[CurrentStack] - addMaxChargeCount[CurrentStack - 1]);
                    }
                ...
                [System.Serializable]
                private class SDashSEData : SoulCardEffectData
                {
                    [BoxGroup("대쉬기 카드", true, true)]
                    [LabelText("대쉬기 사용횟수 증가"), ListDrawerSettings(DraggableItems = false, HideAddButton = true, HideRemoveButton = true, OnBeginListElementGUI = nameof(BeginDrawListElement), OnEndListElementGUI = nameof(EndDrawListElement))]
                    ...
                    {
                        return new SDashSE(this, owner as PlayableUnit);
                    }
